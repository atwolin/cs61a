...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    remain_num_rolls = num_rolls
    score = 0
    is_one = False
    while remain_num_rolls > 0:
        point = dice()
        remain_num_rolls -= 1
        if is_one:
            continue
...
def boar_brawl(player_score, opponent_score):
    ...
    compare = 3 * abs(opponent_tens_digit - player_ones_digit)
    score = max(compare, 1)
    return score
    # END PROBLEM 2
...
def always_roll(n):
    ...
    def strategy(player_score, oppenent_score):
        return n
...
def is_always_roll(strategy, goal=GOAL):
    ...
    player_score = 0
    oppenent_score = 0
    first_num_rolls = strategy(player_score, oppenent_score)
    cur_num_rolls = 0
    while player_score < goal:
        while oppenent_score < goal:
            cur_num_rolls = strategy(player_score, oppenent_score)
            if first_num_rolls != cur_num_rolls:
                return False
            oppenent_score += 1
        player_score += 1
        oppenent_score = 0
...
def make_averaged(original_function, samples_count=1000):
    ...
    def compute_avg_number(*args):
        i = 0
        results_sum = 0
        while i < samples_count:
            results_sum += original_function(*args)
            i += 1
        return results_sum / samples_count

    return compute_avg_number
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    while cur_num_rolls <= 10:
        avg_score = make_averaged(roll_dice, samples_count)(cur_num_rolls, dice)
        if max_score < avg_score:
            max_score = avg_score
            best_num_rolls = cur_num_rolls
        cur_num_rolls += 1
