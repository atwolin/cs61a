...
def double_eights(n):
    ...
    first_two_digit = str(n)[0:2]
    if first_two_digit == "88":
        return True

    return double_eights(str(n)[1:])
...
def mario_number(level):
    ...
    if (
        len(level) == 0
        or (level[0] == "P")
        or (len(level) > 2 and (level[1] == "P" and level[2] == "P"))
    ):
        return 0
...
def max_subseq(n, t):
    ...
    first_section = max_subseq(n // 10, t - 1) * 10 + n % 10
    second_section = max_subseq(n // 10, t)

    return max(first_section, second_section)
